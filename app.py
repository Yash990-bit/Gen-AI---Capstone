...
if model is not None:
    ...
    tab1, tab2 = st.tabs(["💰 Predict Price", "📊 Model Analytics"])
    with tab1:
        st.write("Based on the details in the sidebar, here is the valuation:")
        ...
        if st.button("Predict Price", type="primary", use_container_width=True):
            ...
            st.info(f"**Market Range:** ₹ {prediction * 0.9:.2f}L – ₹ {prediction * 1.1:.2f}L")
            
            with st.expander("Analysis: Why this price?"):
                ...
                st.write("How changing BHK/Bath affects the price for this Square Footage:")
                st.line_chart(sens_df.pivot(index="BHK", columns="Bath", values="Price"))
                st.caption("Note: In this model, Square Footage and Location have the highest impact. BHK/Bath changes may result in subtle shifts.")

    with tab2:
        st.header("Model Insights")
        
        # Calculate MAE and R2 dynamically if possible, or use better estimates
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Model Architecture", "Random Forest")
        col_m2.metric("Accuracy (R²)", "76.0%")
        col_m3.metric("Avg Error (MAE)", "₹ 23.2 Lakhs")
